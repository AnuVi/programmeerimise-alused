...
list_len = len(käärikul)

while (i<list_len):
    #kui kääriku punk on suurem
    if (käärikul[i] > kohilas[i]):
        parim_tulemus.append(käärikul[i])
    #kui kohilas parem
    else:
        parim_tulemus.append(kohilas[i])
    i +=1

#liidame parimad_tulemused kokku
summa = sum(parim_tulemus)
